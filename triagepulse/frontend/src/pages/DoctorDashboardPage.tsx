    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-white tracking-tight">Physician Clinical Portal</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs">
              ...
              >
                <option value="Dr. Michael Vance (Attending Physician)" className="bg-slate-900">Dr. Michael Vance (Attending)</option>
                <option value="Dr. Sarah Thorne (Hospitalist / On-Call)" className="bg-slate-900">Dr. Sarah Thorne (Hospitalist)</option>
                <option value="Dr. Rajiv Patel (Intensivist Consult)" className="bg-slate-900">Dr. Rajiv Patel (ICU Consult)</option>
              </select>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-4 border-t border-slate-800/80">
          <div className="bg-slate-950/60 rounded-xl p-3 border border-slate-800">
            <span className="text-[11px] font-medium text-slate-400">Critical Trajectory Patients</span>
            <div className="flex items-baseline gap-2 mt-1">
              <span className={`text-xl font-bold font-mono ${criticalPatients.length > 0 ? 'text-rose-400' : 'text-slate-200'}`}>
          ...
          <div className="bg-slate-950/60 rounded-xl p-3 border border-slate-800">
            <span className="text-[11px] font-medium text-slate-400">High Risk & Watch</span>
            <div className="flex items-baseline gap-2 mt-1">
          ...
          <div className="bg-slate-950/60 rounded-xl p-3 border border-slate-800">
            <span className="text-[11px] font-medium text-slate-400">Pending Physician Calls</span>
            <div className="flex items-baseline gap-2 mt-1">
          ...
          <div className="bg-slate-950/60 rounded-xl p-3 border border-slate-800">
            <span className="text-[11px] font-medium text-slate-400">Lead Time Advantage</span>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-xl font-bold font-mono text-cyan-400">14.8 min</span>
      ...
      {activeDoctorTab === 'deterioration' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-white">Multi-Vital Deterioration Ranking</h2>
            ...
            {patients.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 bg-slate-950/80 rounded-xl p-2.5 border border-slate-800/80 mb-3">
                    <div>
                      ...
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <span className="text-sm font-bold font-mono text-white">{p.current_vitals.heart_rate}</span>
                        <span className={`text-[10px] font-mono ${hrDeviation > 15 ? 'text-rose-400 font-bold' : 'text-slate-400'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <span className="text-sm font-bold font-mono text-white">{p.current_vitals.spo2}%</span>
                        <span className={`text-[10px] font-mono ${spo2Deviation < -4 ? 'text-rose-400 font-bold' : 'text-slate-400'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-baseline gap-1 mt-0.5">
                        <span className="text-sm font-bold font-mono text-white">{p.current_iv.iv_remaining_ml.toFixed(0)}</span>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px]">
                      <span className="text-slate-400">Attention Priority</span>
                      <span className="font-mono font-bold text-white">{p.trajectory.attention_priority.toFixed(1)} / 100</span>
                    </div>
                  ...
                  <div className="mt-3 pt-2 border-t border-slate-800/80 flex items-center justify-between text-[10px] text-slate-400">
                    ...
                    <span className="text-indigo-400 hover:underline font-semibold">Inspect &rarr;</span>
                  </div>
                </div>
              );
            })}
      ...
      {activeDoctorTab === 'escalations' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-white">Pending Physician Escalations</h2>
              <p className="text-xs text-slate-400">
          ...
          {escalations.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
              ...
              <h3 className="text-base font-bold text-white">No Open Doctor Escalations</h3>
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-white text-base">{esc.patient_name}</span>
                          <span className="px-2 py-0.5 rounded text-[11px] font-mono bg-slate-800 text-slate-300">
                            {esc.room}
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                              isPending ? 'bg-rose-600 text-white animate-pulse' : 'bg-emerald-950 text-emerald-400 border border-emerald-800'
                            }`}
                          >
                            {esc.status}
                          </span>
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                          Escalated by <strong className="text-slate-200">{esc.nurse_name}</strong> • Priority: {esc.priority} • {new Date(esc.created_at).toLocaleTimeString()}
                        </p>
                      </div>
                      ...
                      {matchedPatient && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-800 text-slate-200 hover:bg-slate-700 transition-all flex items-center gap-1.5 self-start"
                        >
                    ...
                    <div className="mt-3 p-3 rounded-xl bg-slate-950/70 border border-slate-800/80 text-xs">
                      ...
                      <p className="text-slate-200 font-medium">{esc.reason}</p>
                      {esc.doctor_notes && (
                        <p className="text-indigo-300 mt-2 pt-2 border-t border-slate-800/60">
                        ...
                        {!isThisResponding ? (
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              className="px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white transition-all shadow-md shadow-indigo-600/20 flex items-center gap-2"
                            >
                          ...
                          <div className="bg-slate-950 border border-indigo-700/60 rounded-xl p-4 space-y-3">
                            <div className="flex items-center justify-between">
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[11px] font-semibold text-slate-400">Select Protocol Order:</label>
                            ...
                            <button
                              ...
                              className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-white transition-all shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2"
                            >
      ...
      {activeDoctorTab === 'sbar' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-2">
            <h3 className="text-sm font-bold text-white mb-2">Select Patient for SBAR</h3>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[11px] mt-1 text-slate-400">
                      ...
                      <span className={`px-1.5 py-0.2 rounded text-[9px] font-bold ${
                        p.trajectory.physiological_level === 'HIGH' ? 'bg-rose-950 text-rose-300' : 'bg-slate-800 text-slate-400'
                      }`}>
                        {p.trajectory.physiological_level}
                      </span>
                    </div>
                ...
                return (
                  <div>
                    <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                      <div>
                        <div className="flex items-center gap-2">
                          <h2 className="text-lg font-bold text-white">{currentP?.name}</h2>
                          <span className="text-xs font-mono text-slate-400">({currentP?.room})</span>
                        </div>
                        <p className="text-xs text-slate-400">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white transition-all flex items-center gap-1.5"
                      >
      ...
      {activeDoctorTab === 'iv_oversight' && (
        <div className="space-y-4">
          <div>
            <h2 className="text-base font-bold text-white">Ward-Wide Intravenous Fluid Oversight</h2>
            <p className="text-xs text-slate-400">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <span className="font-bold text-white text-sm">{p.name}</span>
                      <span className="text-xs text-slate-400 ml-1">({p.room})</span>
                    </div>
                    <span
                  ...
                  <div className="space-y-2 mt-3">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-mono font-bold text-white">{iv.iv_remaining_ml.toFixed(0)} ml / 500 ml</span>
                    </div>

                    <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          iv.iv_remaining_ml < 50 ? 'bg-rose-500' : iv.iv_remaining_ml < 150 ? 'bg-amber-500' : 'bg-cyan-500'
                        }`}
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 text-[11px] text-slate-400">
                      <div>
                        <span>Flow Rate: </span>
                        <strong className="text-slate-200">{iv.iv_flow.toFixed(1)} ml/hr</strong>
                      </div>
                      <div className="text-right">
                        <span>ETA Empty: </span>
                        <strong className={(iv.estimated_time_to_empty_min ?? 999) < 30 ? 'text-rose-400' : 'text-slate-200'}>
