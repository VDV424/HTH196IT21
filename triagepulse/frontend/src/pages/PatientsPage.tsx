      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-white border border-slate-200 p-4 rounded-xl">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-teal-500"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg p-0.5">
            <button
              ...
              className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-slate-100 text-teal-600' : 'text-slate-500'}`}
              title="Card Grid"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded ${viewMode === 'table' ? 'bg-slate-100 text-teal-600' : 'text-slate-500'}`}
              title="Table View"
            >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-500 disabled:opacity-50 text-slate-900 font-medium text-xs transition-colors shadow-sm"
          >
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800 font-mono">
              {filteredPatients.map((p) => (
                <tr key={p.patient_id} className="hover:bg-slate-100/40">
                  <td className="py-3 px-4 font-sans font-bold text-slate-900">{p.patient_id}</td>
                  ...
                  <td className="py-3 px-3 text-teal-600 font-bold">{p.trajectory.attention_priority.toFixed(0)}</td>
                  <td className="py-3 px-3 font-sans">{p.assigned_nurse_name || 'Unassigned'}</td>
                  <td className="py-3 px-4 text-right">
                    <button
                      onClick={() => onSelectPatient(p)}
                      className="px-2.5 py-1 rounded bg-slate-100 hover:bg-teal-600 text-slate-900 font-sans text-xs"
                    >
                      Open
                    </button>
                  </td>
