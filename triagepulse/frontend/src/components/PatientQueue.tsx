    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
      <div className="px-5 py-4 border-b border-slate-800 flex items-center justify-between">
        ...
        <div className="text-xs text-slate-400 flex items-center gap-2">
          ...
          <span className="font-semibold text-white">{patients.length} patients</span>
        </div>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-950/70 text-slate-400 uppercase tracking-wider text-[11px] font-semibold border-b border-slate-800">
            <tr>
              ...
              <th className="py-3 px-3 font-bold text-cyan-400">Attention P(t)</th>
          ...
          <tbody className="divide-y divide-slate-800/60 font-mono">
            {patients.map((p) => {
              ...
              let rowHighlight = 'hover:bg-slate-800/40';
              ...
              if (priority >= 75) {
                ...
                rowHighlight = 'hover:bg-slate-800/40';
              } else if (priority >= 30) {
                ...
                <tr key={p.patient_id} className={`transition-colors duration-150 ${rowHighlight}`}>
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <div className="font-sans font-bold text-sm text-white">{p.patient_id}</div>
                      <span className="text-[10px] text-slate-400 font-sans">({p.room})</span>
                      {isPhysical && (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.2 rounded bg-blue-500/20 text-blue-300 border border-blue-500/30 text-[9px] font-sans">
                          <Radio className="w-2.5 h-2.5" />
                          ESP32
                        </span>
                      )}
                    </div>
                    <div className="text-[10px] text-slate-400 font-sans truncate max-w-[140px]">{p.scenario}</div>
                  </td>
                  ...
                  <td className="py-3 px-3 text-slate-200">
                    <span className={`font-semibold ${p.current_vitals.heart_rate > 105 ? 'text-rose-400' : 'text-slate-200'}`}>
                  ...
                  {/* SpO2 */}
                  <td className="py-3 px-3 text-slate-200">
                    <span className={`font-semibold ${p.current_vitals.spo2 < 93 && p.current_vitals.spo2 > 0 ? 'text-rose-400' : 'text-slate-200'}`}>
                  ...
                  {/* Temperature */}
                  <td className="py-3 px-3 text-slate-200">
                    <span className={`font-semibold ${p.current_vitals.temperature > 37.8 ? 'text-amber-400' : 'text-slate-200'}`}>
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-slate-400 font-sans">({p.trajectory.physiological_level})</span>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-1.5">
                      <Droplets className={`w-3 h-3 ${ivScore >= 60 ? 'text-indigo-400 animate-bounce' : 'text-slate-500'}`} />
                      <span className={`font-semibold ${ivScore >= 60 ? 'text-indigo-300' : 'text-slate-400'}`}>
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-800 hover:bg-cyan-600 text-slate-200 hover:text-white font-sans font-medium text-xs transition-colors"
                    >
                      <span>Open</span>
